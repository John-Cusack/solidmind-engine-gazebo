...
class TestReducedPackageCompiles(unittest.TestCase):
    """A bodies-only package — the shape core writes without a mechanism.

    This used to build the manifest by calling core's builder.  The engine
    cannot import core, and does not need to: the manifest is a published
    format, so a fixture is the honest input.
    """

    def test_reduced_manifest_compiles(self) -> None:
        manifest = _manifest()
        manifest["mode"] = "reduced"
        manifest.pop("joints")
        manifest["joints"] = []
        manifest.pop("actuators", None)
        manifest.pop("sensors", None)
        for link in manifest["links"]:
            link.pop("collision", None)
            if "mesh" in link:
                link["mesh"]["frame"] = "world"

        pkg = _Package(manifest)
        self.addCleanup(pkg.cleanup)
        model = ET.parse(compile_package_to_sdf(str(pkg.dir))).getroot().find("model")
